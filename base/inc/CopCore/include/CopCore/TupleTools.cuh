...
struct TupleElementsNotIn<std::tuple<T, Elements...>, OtherTuple> {
  ...
  using t          = typename std::conditional_t<TupleContains<T, OtherTuple>::value, previous_t,
...
struct tuple_ref_index<Base, std::tuple<Head, Tail...>, I>
    : std::conditional<std::is_base_of<typename std::decay<Base>::type, typename std::decay<Head>::type>::value,
